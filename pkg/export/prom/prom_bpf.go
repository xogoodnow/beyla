...
func (bc *BPFCollector) collectMapMetrics(ch chan<- prometheus.Metric) {
	...
	for ids != 0 {
		...
		if err := iter.Err(); err != nil {
			bc.log.Error("failed to iterate over map", "ID", ids, "error", err)
		} else {
...
func (bp *BPFProgram) updateBuckets() {
	...
	if deltaCount > 0 {
		avgLatency = time.Duration(deltaTime.Seconds() / float64(deltaCount))
	} else {
		avgLatency = 0
	}
	...
	for _, bucket := range bucketKeysSeconds {
		if avgLatency < time.Duration(bucket) {
